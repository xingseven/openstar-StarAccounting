...
function LoadingMetricShell() {
  return (
    <div className="rounded-xl [background:var(--theme-metric-bg)] p-3 sm:rounded-xl sm:p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1 space-y-2">
          <Skeleton className="h-3.5 w-20 rounded-full bg-white/70 sm:h-4" />
          <Skeleton className="h-7 w-24 rounded-lg bg-white/85 sm:h-8 sm:w-28" />
...
function LoadingSurfaceShell({ className }: { className?: string }) {
  return (
    <section
      className={`relative overflow-hidden rounded-xl [background:var(--theme-surface-bg)] sm:rounded-2xl ${className ?? ""}`}
    >
      <div className="space-y-4 p-3.5 sm:p-6">
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-2">
            <Skeleton className="h-4 w-24 rounded-full" />
            <Skeleton className="h-7 w-40 rounded-lg sm:h-8 sm:w-52" />
        ...
        <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_180px]">
          <div className="flex h-[220px] items-end gap-2 rounded-xl bg-slate-50/85 p-4 sm:h-[280px]">
          ...
          <div className="space-y-2.5">
            {Array.from({ length: 4 }).map((_, index) => (
              <div
                ...
                className="flex items-center justify-between gap-3 rounded-xl bg-slate-50/85 px-3 py-3"
              >
...
export function ConsumptionLoadingShell() {
  return (
    <div aria-busy="true" aria-live="polite" className="mx-auto max-w-[1680px] space-y-4 pb-2 sm:space-y-5">
      <section className="relative overflow-hidden rounded-2xl [background:var(--theme-hero-bg)] p-3.5 sm:rounded-[26px] sm:p-6 lg:p-8">
        ...
        <div className="relative z-10 grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.92fr)]">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap items-end gap-x-6 gap-y-3">
                <div className="space-y-2">
                  ...
                  <Skeleton className="h-11 w-40 rounded-xl bg-white/80 sm:h-14 sm:w-48" />
                </div>
                <Skeleton className="h-9 w-36 rounded-full bg-white/70" />
              </div>
            </div>

            <div className="overflow-hidden rounded-2xl bg-slate-200/70 ring-1 ring-slate-200/70 sm:grid sm:grid-cols-3 sm:[&>*:not(:last-child)]:border-r sm:[&>*:not(:last-child)]:border-slate-200/70">
          ...
          <div className="self-end xl:justify-self-end">
            <div className="grid gap-2.5 sm:min-w-[320px]">
              <div className="rounded-2xl bg-white/72 p-4 ring-1 ring-white/65">
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-2">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <Skeleton className="h-11 rounded-xl bg-white" />
                  <Skeleton className="h-11 rounded-xl bg-white" />
                  <Skeleton className="h-11 rounded-xl bg-white sm:col-span-2" />
                </div>
              </div>
            </div>
          </div>
