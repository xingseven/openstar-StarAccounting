...
- **新增 Dashboard 拖拽布局专项开发文档**:
  - 新建 `docs/Dashboard拖拽布局开发文档.md`，明确当前项目如需支持图表模块拖拽自定义布局，应优先走“栅格拖拽布局引擎 + 普通主题卡片 DOM”方案，而不是把 Dashboard 背景层直接改成自由画布。
  - 文档中补齐了推荐库选型、widget 注册表、布局数据结构、编辑模式、布局持久化、移动端降级、性能要求和分阶段实施计划。
  - 更新 `docs/主题开发框架文档.md`，新增“布局层只管理位置尺寸，主题层继续负责页面壳和卡片表面”的规则。
  - 更新 `docs/开发进度.md`，新增 V2.3.101 记录。
  - `docs/主题开发框架文档.md` 小版本升级到 `v2.1.21`。

### Verified

- `git diff --check -- docs/Dashboard拖拽布局开发文档.md docs/主题开发框架文档.md docs/开发进度.md CHANGELOG.md`
...
- **同步主题框架文档的迁移边界说明**:
  - 更新 `docs/主题开发框架文档.md`，补充当前正式生效的前端目录为 `web/src`、后端目录为 `server/src`。
  - 明确根目录旧 `src/` 只是迁移后的历史残留，不再允许继续承载正式源码。
  - 清理本地残留的空 `src/server` 目录，避免继续误导开发路径判断。

### Docs

- **版本记录同步**:
  - 更新 `docs/开发进度.md`，新增 V2.3.89 记录。
...
- **共享悬浮筛选器补齐非受控交互**:
  ...
  - 当父页面只传 `isOpen`、`onOpenChange` 等基础参数而未传筛选回调时，浮层会自动回退到内部状态，避免总览页这类接入场景出现按钮点击无响应。
  - 默认主题总览页复用共享筛选器后，浮层内的“本月 / 全部时间 / 自定义”以及年份、月份切换按钮现在都可以正常交互。

### Docs

- **主题文档与版本记录同步**:
  - `docs/主题开发框架文档.md` 小版本升级到 `v2.1.6`，补充共享悬浮筛选器的接入约定。
  - `docs/开发进度.md`、`CHANGELOG.md` 同步记录本次共享组件修复。
