...
- **Flutter 统一前端起步工程与总览页首版**:
  - 新增 `app_flutter/` Flutter 工程，接入 `go_router`、`flutter_riverpod`、`dio` 与 `intl`，建立启动入口、环境配置、认证状态、路由守卫和响应式壳层。
...
- **Flutter 第一阶段初始化执行清单**:
  ...
  - 文档补充了环境准备、首日命令、依赖安装顺序、推荐目录结构、首个接口接入顺序、第一周计划、验收标准和常见错误，便于按步骤真正启动 `app_flutter/`。
...
- **数据页工作区重排**:
  - 顶部改为“总记录数 + 账单导入”同排展示，减少重复信息块。
  - PC 端进一步整理为双列工作区，左侧放总记录数与手动补录，右侧放账单导入与自动归类，整体阅读路径更清晰。
- **AI 页面交互收口**:
  - 顶部导航栏改为直接显示“AI 模型配置”，删除页面内部重复的大标题模块。
  - 已配置模型中的齿轮菜单修复裁切问题，菜单项保持单行显示，配置弹层内容区支持滚动。
- **弹层与分析结果动效优化**:
  - 底部滑出页改为更顺滑的上滑进入与下滑退出，遮罩与面板过渡节奏统一。
  - 消费页 AI 分析卡在初始态、加载态、错误态和结果态之间补齐渐入渐出过渡，减少硬切换感。
...
- **消费页规则备注分析收口**:
  - 将消费页“规则备注”统计整合为单张分析卡，统一展示覆盖统计、备注金额分布与 Top 备注排行。
  - 规则备注图表改为优先按自动归类规则里的备注或规则名聚合，不再直接混入原始订单描述。
- **跨页面交易明细样式统一**:
  - 抽离共享的紧凑型单行流水行组件，并统一应用到消费、总览、储蓄与数据页面的明细区。
  - 消费页底部流水改为仅展示最近 5 条，整体列表压缩为分割线样式，减少页面纵向占用。
- **消费页细节修正**:
  - 修复消费日历按月视图中月份后缀乱码问题。
  - 消费页底部流水时间改为更易读的本地短格式显示。
...
- **非 Docker 自动部署链路**:
  - 新增 `.github/workflows/deploy-non-docker.yml`，在 `main` 分支推送后可自动校验并通过 SSH 执行远程部署。
  - 新增 `src/server/scripts/deploy-linux.sh` 与 `src/server/scripts/deploy-windows.ps1`，统一 Linux / Windows 服务器上的拉取、构建、PM2 重启与健康检查流程。
  - 新增 `docs/非Docker自动部署开发文档.md`，整理目标、前置条件、Secrets 规划与脚本职责边界。

### Modified

- **账单导入按订单号强约束去重**:
  - 微信账单按“交易单号”、支付宝账单按“交易订单号”映射 `orderId`，导入时缺少订单号的记录直接记为无效行。
  - 导入阶段继续按 `orderId` 跳过数据库已存在记录与同批文件内重复记录，确保同一订单不会重复入库。
- **数据页分类目录与录入建议联动**:
  - 新增 `web/src/lib/transaction-categories.ts`，统一维护收入/支出分类目录并复用到数据页与消费页。
  - 数据页手动补录、批量改分类、自动归类规则创建与导入完成后会同步刷新分类目录，输入框支持直接联想最近真实分类。
- **消费页规则备注分析增强**:
  - 消费分析新增“规则备注”统计、Top 备注与金额分布，优先展示自动归类规则沉淀下来的固定场景支出。
  - 交易明细支持搜索备注，并新增“只看有备注”筛选，便于快速审查已标注流水。
  - 交易明细时间改为 `MM-DD HH:mm` 的短格式显示，减少长时间串对窄屏列表的挤压。
- **AI 模型配置内存模式补齐**:
  - `/api/ai/models` 在无数据库时补齐内存态的增删改查与默认模型读取逻辑，不再出现列表可打开但创建直接 500 的情况。

### Fixed

- **总览资产统计避免重复计算储蓄**:
  - 总览页汇总总资产时，已同步到资产账户的储蓄目标不再重复叠加到资产合计。
- **资产页与储蓄页空数据回退修正**:
  - 资产页、储蓄页在真实空数据或接口加载失败时不再强行回退 mock 数据，改为返回空列表并展示真实空态。
...
- **复购与集中度图标签裁切修正**:
  - 继续调整消费页“复购与集中”模块中集中度条形图的右侧留白与 X 轴范围，确保右侧百分比标签完整显示。
...
- **消费交易明细乱码修复**:
  - 修复消费页底部交易明细卡片中“收入 / 支出”状态文案的乱码显示问题。
- **消费分析维度批量扩展**:
  - 为消费页新增消费属性、必要/可选、资金性质、高频商户/订阅、预算偏差、高峰场景、周末偏好、大额预警和消费集中度等多组分析维度。
  - 扩展 `/api/consumption/dashboard` 聚合结果与 mock 数据，使新增分析模块在真实数据和演示数据下都能展示。
- **消费分析维度图表化收敛**:
  - 将“更多分析维度”从信息卡列表改为少量图表卡，统一收成结构视图、复购与集中度、预算与风险、场景偏好四组图表面板。
- **消费分析条形图标签防裁切**:
  - 调整预算偏差、大额预警、时段热点、周末偏好等条形图的右侧留白和 X 轴范围，避免数值标签超出容器。
...
- **数据页新增交易对方自动归类规则**:
  - 支持从交易对方去重列表中搜索、勾选并批量建立规则。
  - 规则可绑定到房租等自定义分类，并可选统一备注与回填历史交易。

### Modified

- **交易入库流程接入自动归类**:
  - 手动新增交易、账单导入和 APP 同步推送都会按交易对方规则自动套用分类。
  - 后续新增同一交易对方的支出时，无需再次手动改分类。

- **AI 模型配置切换到当前账户作用域**:
  - 修复 AI 页面添加大模型时因 `accountId` 写入错误导致只提示“创建失败”的问题。
  - AI 模型列表、更新、删除以及默认模型读取逻辑统一按当前账户隔离。

- **AI 页面概览卡片在 PC 端统一单行**:
  - 模型总数、已配置、未配置和功能说明四个模块在大屏下改为同一行展示。
