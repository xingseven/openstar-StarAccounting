...
export type ImportResult = {
  ...
  headers: string[];
  rows: ImportRow[];
  encoding: "xlsx" | "utf8" | "gbk" | "utf16le" | "utf16be";
};
...
function splitLenientLine(line: string, delimiter: (typeof CSV_DELIMITERS)[number]) {
  const cells: string[] = [];
  let current = "";
  let inQuotes = false;
...
function parseRowsLenient(csvText: string, source: Source) {
  const cleanText = csvText.replace(/^\uFEFF/, "");
  ...
  for (const delimiter of CSV_DELIMITERS) {
    ...
    if (candidate.headerIndex < 0) continue;
...
function getHeaderFeatures(source: Source) {
  if (source === "wechat") {
...
function findHeaderCandidate(rows: string[][], source: Source): HeaderCandidate {
...
function hasUtf16NullPattern(buffer: Buffer) {
...
function detectLegacyEncoding(buffer: Buffer) {
  if (startsWithBytes(buffer, UTF8_BOM)) return "utf8" as const;
  if (startsWithBytes(buffer, UTF16LE_BOM)) return "utf16le" as const;
  if (startsWithBytes(buffer, UTF16BE_BOM)) return "utf16be" as const;
  ...
  if (hasGbkChars > 10) return "gbk" as const;

  const utf8Probe = probe.toString("utf8");
  const replacementCount = (utf8Probe.match(/�/g) || []).length;
  if (replacementCount > 2) return "gbk" as const;

  return "utf8" as const;
}

function getEncodingCandidates(buffer: Buffer): ImportResult["encoding"][] {
  const candidates: ImportResult["encoding"][] = [];
...
function decodeBuffer(buffer: Buffer, encoding: ImportResult["encoding"]) {
  switch (encoding) {
    case "utf8":
...
function buildCandidate(
  rows: string[][],
  source: Source,
  encoding: ParsedCandidate["encoding"],
): ParsedCandidate | null {
  const { headerIndex, headerScore } = findHeaderCandidate(rows, source);
  if (headerIndex < 0 || headerScore < 2) return null;
...
function pickBetterCandidate(current: ParsedCandidate | null, next: ParsedCandidate | null) {
