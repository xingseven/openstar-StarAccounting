...
import { calculateBudgetUsage, calculateBudgetHealth } from "./logic/budget.js";
import { calculateAssetValue } from "./logic/asset.js";
import { fetchExchangeRates } from "./services/exchangeRate.js";
import { scanReceipt, analyzeConsumption } from "./services/doubaoAi.js";
const connectionsById = new Map();
const connectionIdByOtp = new Map();
const transactionsByUser = new Map();
const transactionsByAccount = new Map();
const merchantCategoryRulesByAccount = new Map();
const savingsGoalsByUser = new Map();
const loansByUser = new Map();
const assetsByUser = new Map();
const budgetsByUser = new Map();
const exchangeRates = new Map();
const aiModelConfigsByAccount = new Map();
...
function normalizeSavingsPlanConfig(value) {
...
function getSavingsAssetSyncConfig(value) {
...
function getSavingsPlanAssetIds(value) {
    const { sourceAssetId, holdingAssetId } = getSavingsAssetSyncConfig(value);
    return Array.from(new Set([sourceAssetId, holdingAssetId].filter((item) => Boolean(item))));
}
async function ensureSavingsPlanAssetsOwnedInDb(prisma, userId, value) {
...
function ensureSavingsPlanAssetsOwnedInMemory(userId, value) {
    const assetIds = getSavingsPlanAssetIds(value);
    if (assetIds.length === 0)
        return;
...
async function applySavingsAssetDeltaInDb(tx, userId, planConfig, deltaAmount) {
    if (!Number.isFinite(deltaAmount) || deltaAmount === 0)
        return;
    const assetSync = getSavingsAssetSyncConfig(planConfig);
    if (!assetSync.syncToAssets)
        return;
    const sourceAssetId = assetSync.sourceAssetId;
    const holdingAssetId = assetSync.holdingAssetId;
    if (!sourceAssetId && !holdingAssetId)
        return;
    if (sourceAssetId && holdingAssetId && sourceAssetId === holdingAssetId)
        return;
    const assetIds = Array.from(new Set([sourceAssetId, holdingAssetId].filter((item) => Boolean(item))));
...
function applySavingsAssetDeltaInMemory(userId, planConfig, deltaAmount) {
    if (!Number.isFinite(deltaAmount) || deltaAmount === 0)
        return;
    const assetSync = getSavingsAssetSyncConfig(planConfig);
    if (!assetSync.syncToAssets)
...
function jsonFail(res, status, code, message, detail) {
    const body = { code, message, detail };
...
function getPublicConnectHost(req) {
...
function findExistingFileByRelativePath(relativePath) {
...
function parseUpdateManifest(input) {
...
function resolveLocalPublicAssetPath(assetUrl) {
...
async function requireAccountId(req, res) {
    const authContext = await getAuthContext(req);
...
function normalizeLoanMatchText(value) {
    return (value ?? "")
        .toLowerCase()
        .replace(/[\s()（）【】\[\]{}\-_/&、·.]/g, "")
        .trim();
...
function expandLoanAlias(term) {
    ...
    if (normalized.includes("花呗"))
        set.add("花呗");
    if (normalized.includes("借呗"))
        set.add("借呗");
    if (normalized.includes("备用金"))
        set.add("备用金");
    return Array.from(set).filter((item) => item.length >= 2);
}
function collectLoanMatchTerms(loan) {
...
function isRepaymentLikeTransaction(transaction) {
...
function findMatchedLoanForRepaymentTransaction(transaction, loans) {
    if (!isRepaymentLikeTransaction(transaction))
        return null;
...
function canAutoSyncImportedRepayment(loan) {
    return loan.paidPeriods === 0 && Math.abs(loan.remainingAmount - loan.totalAmount) < 0.000001;
}
function computeReconciledLoanState(loan, transactions) {
...
function toAIModelResponse(model) {
    return {
        id: model.id,
        name: model.name,
...
function getAIModelsFromMemory(accountId) {
    return aiModelConfigsByAccount.get(accountId) ?? [];
}
function getActiveAIModelConfigFromMemory(accountId) {
...
async function getPreferredAIModelConfig(accountId, prisma) {
    if (prisma) {
...
function normalizeMerchantRuleKey(value) {
...
let ensureMerchantCategoryRuleTablePromise = null;
async function ensureMerchantCategoryRuleTable(prisma) {
    if (ensureMerchantCategoryRuleTablePromise)
...
function getMerchantCategoryRulesFromMemory(accountId) {
    return (merchantCategoryRulesByAccount.get(accountId) ?? []).filter((rule) => rule.isActive);
}
async function getMerchantCategoryRules(prisma, userId, accountId) {
...
function applyMerchantCategoryRules(input, rules) {
...
function addTransactionCategory(catalog, category, type) {
...
function finalizeTransactionCategoryCatalog(catalog) {
    const sortByLocale = (left, right) => left.localeCompare(right, "zh-CN");
    return {
        income: Array.from(catalog.income).sort(sortByLocale),
        expense: Array.from(catalog.expense).sort(sortByLocale),
    };
}
function resolveRuleRemarkLabel(merchant, rulesByMerchantKey) {
...
function decodeSyncCursor(value) {
    ...
    try {
        ...
        if (typeof raw.updatedAt !== "string" || typeof raw.id !== "string")
            return null;
        if (Number.isNaN(new Date(raw.updatedAt).getTime()))
            return null;
...
function compareSyncRecordCursor(record, cursor) {
    if (!cursor)
        return true;
    const value = new Date(record.updatedAt ?? record.date).toISOString();
    if (value > cursor.updatedAt)
        return true;
    if (value < cursor.updatedAt)
        return false;
    return record.id > cursor.id;
}
function normalizeSyncTransactionPayload(input) {
    ...
    const amountValue = typeof input.amount === "number" || typeof input.amount === "string" ? Number(input.amount) : Number.NaN;
    const type = typeof input.type === "string" ? input.type.trim() : "";
    if (!id && !orderId) {
        return { ok: false, clientId, error: "新交易至少需要提供 id 或 orderId" };
    }
    if (!date || Number.isNaN(date.getTime())) {
        return { ok: false, clientId, error: "date 格式不正确" };
    }
    if (!Number.isFinite(amountValue)) {
        return { ok: false, clientId, error: "amount 格式不正确" };
    }
    if (!isTransactionType(type)) {
        return { ok: false, clientId, error: "type 必须是 INCOME/EXPENSE/TRANSFER/REPAYMENT" };
    }
    if (!category) {
        return { ok: false, clientId, error: "category 不能为空" };
    }
    if (!platform) {
        return { ok: false, clientId, error: "platform 不能为空" };
    }
    return {
        ok: true,
        clientId,
...
const DASHBOARD_PLATFORM_COLORS = {
    wechat: "#2563eb",
    alipay: "#1d4ed8",
    cloudpay: "#3b82f6",
    unknown: "#93c5fd",
...
const CONSUMPTION_INSIGHT_COLORS = ["#1d4ed8", "#2563eb", "#3b82f6", "#60a5fa", "#93c5fd", "#bfdbfe"];
...
function normalizeInsightText(...values) {
    return values
        .map((value) => (value ?? "").trim().toLowerCase())
        .filter(Boolean)
        .join(" ");
}
function classifyNeedType(category, merchantText) {
...
function classifyTransactionNature(row, category, merchantText) {
...
function resolveTimeBucket(date) {
...
function resolveBudgetInsightPeriod(start, end) {
    if (!start || !end)
        return null;
...
function getBudgetInsightName(budget) {
...
function formatMetricBucket(date, groupBy = "day") {
...
function aggregateDailyMetrics(rows, groupBy = "day") {
    const map = new Map();
...
function aggregateCashflowMetrics(rows, groupBy = "day") {
    const map = new Map();
    for (const row of rows) {
        const date = row.date instanceof Date ? row.date : new Date(row.date);
        if (Number.isNaN(date.getTime()))
...
function aggregateDailyCategoryMetrics(rows, groupBy = "day") {
    const map = new Map();
    for (const row of rows) {
        const date = row.date instanceof Date ? row.date : new Date(row.date);
        if (Number.isNaN(date.getTime()))
...
function normalizeDashboardPlatform(platform) {
    const value = (platform ?? "").trim().toLowerCase();
    if (!value)
        return "unknown";
    if (value === "wechat" || value.includes("微信"))
        return "wechat";
    if (value === "alipay" || value.includes("支付宝"))
        return "alipay";
    if (value === "cloudpay"
        || value === "unionpay"
        || value.includes("云闪付")
...
function buildConsumptionDashboard(rows, bucket = "day") {
    const expensePlatformMap = new Map();
    const incomePlatformMap = new Map();
    ...
    const incomeExpense = [
        { name: "支出", value: totalExpense, fill: "#1d4ed8" },
        { name: "收入", value: totalIncome, fill: "#60a5fa" },
    ];
    ...
    const stackedBar = trend.map((item) => {
        const row = { day: item.day };
        for (const category of stackedCategories) {
            const found = dailyCategoryItems.find((dailyCategoryItem) => dailyCategoryItem.day === item.day && dailyCategoryItem.category === category);
    ...
    const sankeyLinks = heatmap.data
        ...
        if (source === undefined || targetIndex < 0)
            return null;
        return {
            source,
...
function buildConsumptionDashboardExtended(rows, bucket = "day", budgets = [], range, merchantRules = []) {
    const base = buildConsumptionDashboard(rows, bucket);
    const ruleRemarkMap = new Map(merchantRules
    ...
    const weekendCategoryMap = new Map();
    ...
    const recurringMerchantCandidates = Array.from(merchantStats.entries())
        ...
        const isSubscription = includesAnyKeyword(stat.merchantText, SUBSCRIPTION_KEYWORDS)
            || (avgInterval !== null && avgInterval >= 23 && avgInterval <= 40 && stat.count >= 2);
        if (stat.count < 2 && !isSubscription)
            return null;
        return {
            merchant,
            total: stat.total,
        ...
        .filter((item) => item !== null)
        .sort((a, b) => {
        if (Number(b.isSubscription) !== Number(a.isSubscription))
            return Number(b.isSubscription) - Number(a.isSubscription);
        if (b.count !== a.count)
            return b.count - a.count;
        return b.total - a.total;
    });
    const recurringMerchantSet = new Set(recurringMerchantCandidates
        .filter((item) => item.isSubscription || item.count >= 3)
        .map((item) => item.merchant));
    const spendingStyleTotals = { fixed: 0, flexible: 0, impulse: 0 };
    for (const row of expenseRows) {
        const fixedLike = recurringMerchantSet.has(row.merchant)
            || includesAnyKeyword(row.merchantText, FIXED_KEYWORDS)
            || includesAnyKeyword(row.category, ["居住", "教育", "医疗", "生活"]);
        const impulseLike = includesAnyKeyword(row.merchantText, IMPULSE_KEYWORDS)
    ...
    const transactionNatureBase = Math.max(natureTotals.consumption + natureTotals.transfer + natureTotals.refund, 1);
    ...
    const weekendPreference = Array.from(weekendCategoryMap.entries())
        ...
        .sort((a, b) => {
        if (b.weekendShare !== a.weekendShare)
            return b.weekendShare - a.weekendShare;
    ...
    const largeExpenses = expenseRows
        ...
        reason: row.localDate.getHours() >= 22 || row.localDate.getHours() < 6
            ? "夜间大额"
    ...
    const concentration = {
        ...
        top3MerchantShare: base.summary.totalExpense > 0
    ...
    return {
        ...
        insights: {
            ...
            recurringMerchants: recurringMerchantCandidates.slice(0, 4).map((item) => ({
                ...
                total: item.total,
                count: item.count,
                cadenceLabel: item.avgInterval !== null
...
function matchesTransactionSearch(item, search) {
    if (!search)
        return true;
...
function filterTransactionsInMemory(items, params) {
    return items.filter((item) => {
        ...
        if (params.start && ts < params.start.getTime())
            return false;
        if (params.end && ts > params.end.getTime())
            return false;
        return matchesTransactionSearch(item, params.search);
    });
}
async function queryConsumptionSummaryWithPrisma(prisma, where) {
...
function queryConsumptionSummaryInMemory(items, params) {
...
function buildBudgetHealthPayload(budgets, transactions, now = new Date()) {
    ...
    return budgets.map((budget) => {
        const health = calculateBudgetHealth({
            ...
            scopeType: budget.scopeType === "CATEGORY" || budget.scopeType === "PLATFORM"
                ? budget.scopeType
                : "GLOBAL",
            platform: budget.platform ?? undefined,
            alertPercent: budget.alertPercent ?? undefined,
        }, normalizedTransactions, now);
        return {
            id: budget.id,
            category: budget.category,
...
app.get("/api/dashboard/summary", async (req, res) => {
    ...
    if (prisma) {
        try {
            ...
            const [assetsData, ratesData, loansData, savingsData, currentExpenseSummary, currentIncomeSummary, compareExpenseSummary, compareIncomeSummary, recentTransactions, budgets, budgetTransactions,] = await Promise.all([
            ...
            const totalAssets = assetsData.reduce((sum, item) => sum + Number(calculateAssetValue(item, "CNY", rateItems)), 0) +
                savingsData.reduce((sum, item) => sum + (getSavingsAssetSyncConfig(item.planConfig).syncToAssets ? 0 : Number(item.currentAmount)), 0);
            ...
            const savingsTransactions = recentItems.filter((item) => savingsKeywords.some((keyword) => item.category?.includes(keyword) || item.merchant?.includes(keyword)));
            const budgetAlerts = buildBudgetHealthPayload(budgets.map((item) => ({
                id: item.id,
                amount: item.amount,
                category: item.category,
                period: item.period,
            ...
            })), budgetTransactions, now).filter((item) => item.status !== "normal");
            jsonOk(res, {
                totalAssets,
    ...
    const budgetAlerts = buildBudgetHealthPayload((budgetsByUser.get(userId) ?? []).map((item) => ({
        id: item.id,
        amount: item.amount,
        category: item.category,
        period: item.period,
        ...
        platform: item.platform,
        date: item.date,
    })), now).filter((item) => item.status !== "normal");
    ...
    const savingsTransactions = recentItems.filter((item) => savingsKeywords.some((keyword) => item.category?.includes(keyword) || item.merchant?.includes(keyword)));
    jsonOk(res, {
        totalAssets: assets.reduce((sum, item) => sum + Number(calculateAssetValue(item, "CNY", rateItems)), 0) +
        ...
        lastMonthExpense: Number(queryConsumptionSummaryInMemory(allTransactions, {
            start: compareStart,
            end: compareEnd,
            type: "EXPENSE",
            platform,
        ...
        lastMonthIncome: Number(queryConsumptionSummaryInMemory(allTransactions, {
            start: compareStart,
            end: compareEnd,
            type: "INCOME",
            platform,
...
app.get("/api/consumption/dashboard", async (req, res) => {
    ...
    jsonOk(res, buildConsumptionDashboardExtended(filtered, bucket, (budgetsByUser.get(userId) ?? []).map((item) => ({
        amount: Number(item.amount),
        category: item.category,
        period: item.period,
    })), { start: start ?? undefined, end: end ?? undefined }, getMerchantCategoryRulesFromMemory(account.accountId)));
});
...
app.get("/api/metrics/consumption/daily-category", async (req, res) => {
    ...
    jsonOk(res, {
        items: aggregateDailyCategoryMetrics(filtered.map((t) => ({
            date: t.date,
            category: t.category,
            amount: t.amount,
        }))),
    ...
    if (prisma) {
        try {
            ...
            const rows = await prisma.transaction.groupBy({
                ...
                where: buildPrismaTransactionWhere({ userId, accountId, start, end, type }),
                _sum: { amount: true },
            });
...
app.get("/api/metrics/consumption/daily", async (req, res) => {
    ...
    jsonOk(res, {
        items: aggregateDailyMetrics(filtered.map((t) => ({
            date: t.date,
            amount: t.amount,
        })), groupBy),
    });
...
app.get("/api/transactions", async (req, res) => {
    ...
    const items = filtered.slice((Math.max(1, page) - 1) * pageSize, Math.max(1, page) * pageSize);
    jsonOk(res, { page, pageSize, total, items });
});
app.get("/api/transactions/merchants", async (req, res) => {
    ...
    if (prisma) {
        try {
          ...
          LIMIT ${limit}
        `, userId, accountId, likeKeyword, likeKeyword);
            jsonOk(res, {
                items: rows.map((row) => ({
                    merchant: row.merchant,
    ...
    for (const item of transactionsByAccount.get(accountId) ?? transactionsByUser.get(userId) ?? []) {
        ...
        if (!merchant)
            continue;
        if (keyword && !merchant.toLocaleLowerCase("zh-CN").includes(keyword.toLocaleLowerCase("zh-CN")))
            continue;
...
app.post("/api/transactions/rules", async (req, res) => {
    ...
    const normalizedMerchants = Array.from(new Map(merchants
        .map((merchant) => normalizeOptionalText(merchant))
        .filter((merchant) => Boolean(merchant))
        .map((merchant) => [normalizeMerchantRuleKey(merchant) ?? merchant, merchant])).values());
    ...
    if (prisma) {
        try {
          ...
          `, crypto.randomUUID(), userId, accountId, name, merchant, merchantKey, category, description, now);
            }
    ...
    if (applyToHistory) {
        ...
        const updatedList = list.map((transaction) => {
            if (transaction.type !== "EXPENSE")
                return transaction;
            if (!merchantSet.has(normalizeMerchantRuleKey(transaction.merchant)))
                return transaction;
            historyUpdated += 1;
            return {
                ...transaction,
...
app.post("/api/transactions/import", upload.single("file"), async (req, res) => {
    ...
    try {
        ...
        const code = typeof e === "object" && e && "code" in e ? e.code : 50000;
        if (code === 30001) {
    ...
    if (prisma) {
        try {
            ...
            const orderIds = ruleAppliedValid.map((v) => v.orderId).filter((v) => !!v);
            ...
            const existing = orderIds.length > 0
                ...
                : [];
            const existingSet = new Set(existing.map((e) => e.orderId).filter((v) => !!v));
            ...
            const loanStateMap = new Map(loanCandidates.map((loan) => [
                loan.id,
                {
    ...
    const existingSet = new Set(list.map((t) => t.orderId).filter((v) => !!v));
...
app.get("/api/sync/transactions/pull", async (req, res) => {
    ...
    const updatedAfterParam = typeof req.query.updatedAfter === "string" ? req.query.updatedAfter : undefined;
    const cursor = decodeSyncCursor(cursorParam) ??
...
app.post("/api/connect/verify", async (req, res) => {
    ...
    const normalizedDeviceName = typeof deviceName === "string" && deviceName.trim().length > 0 ? deviceName.trim() : null;
            ...
            const result = await prisma.$transaction(async (tx) => {
                ...
                let goal = await tx.savingsgoal.create({
                    data: {
                        ...
                        planConfig: (normalizedPlanConfig ?? undefined),
                        updatedAt: new Date(),
                    },
                });
                ...
                const completedAmount = createdPlans.reduce((sum, plan) => sum + (plan.status === "COMPLETED" ? Number(plan.amount ?? 0) : 0), 0);
            ...
            await prisma.$transaction(async (tx) => {
                ...
                await tx.savingsgoal.update({
                    ...
                    data: {
                        ...
                        ...(normalizedConfig !== undefined ? { planConfig: normalizedConfig } : {}),
                    },
                });
            });
    ...
    if (prisma) {
        try {
            const result = await prisma.$transaction(async (tx) => {
                ...
                const loanCandidates = allLoans.map((item) => ({
                    id: item.id,
                    platform: item.platform,
        ...
        try {
            ...
            const asset = await prisma.asset.create({
                data: {
                    ...
                    accountId,
                    name,
                    type: type, // Simplified type casting for now
...
app.get("/api/budgets", async (req, res) => {
    ...
    if (prisma) {
        try {
            ...
            const items = buildBudgetHealthPayload(budgets.map((item) => ({
                id: item.id,
                amount: item.amount,
                category: item.category,
                period: item.period,
                ...
                platform: item.platform,
                alertPercent: item.alertPercent,
            })), expenseTransactions, now).map((item) => ({
                id: item.id,
                category: item.category,
                platform: item.platform,
                period: item.period,
    ...
    const items = budgets.map((b) => {
        const health = calculateBudgetHealth({ ...b, amount: Number(b.amount) }, transactions.map(t => ({ ...t, amount: Number(t.amount), date: new Date(t.date) })), now);
        return {
            ...b,
            used: health.used.toFixed(2),
...
app.post("/api/budgets", async (req, res) => {
    ...
    const exists = list.some((b) => b.category === (category || "ALL") && b.period === (period || "MONTHLY"));
    if (exists) {
        jsonFail(res, 400, 50000, "DUPLICATE_BUDGET", "该分类在此周期下已存在预算");
        return;
    }
...
app.get("/api/budgets/alerts", async (req, res) => {
    ...
    if (prisma) {
        try {
            ...
            const alerts = buildBudgetHealthPayload(budgets.map((item) => ({
                id: item.id,
                amount: item.amount,
                category: item.category,
                period: item.period,
                ...
                platform: item.platform,
                alertPercent: item.alertPercent,
            })), expenseTransactions, now).filter((item) => item.status !== "normal");
            jsonOk(res, { alerts });
            return;
        }
        catch (e) {
...
app.post("/api/ai/scan-receipt", upload.single("image"), async (req, res) => {
    try {
        ...
        // 获取平台参数（默认支付宝）
        const platform = req.body?.platform || "alipay";
        ...
        // 调用 AI 进行识别（传入配置和平台）
        const result = await scanReceipt(imageBase64, modelConfig || undefined, platform);
        // 返回结果，包含平台特有字段
        jsonOk(res, {
...
app.post("/api/ai/analyze-consumption", async (req, res) => {
    try {
        ...
        if (!account)
            return;
        const { accountId } = account;
        const { transactions, budgets, startDate, endDate } = req.body;
        ...
        const prisma = getPrisma();
        const modelConfig = await getPreferredAIModelConfig(accountId, prisma);
        const result = await analyzeConsumption(transactions, budgets || [], modelConfig || undefined);
        jsonOk(res, result);
    }
    catch (error) {
...
app.get("/api/ai/models", async (req, res) => {
    ...
    try {
        ...
        const safeModels = models.map((model) => toAIModelResponse({
            id: model.id,
            name: model.name,
            provider: model.provider,
...
app.post("/api/ai/models", async (req, res) => {
    ...
    const normalizedType = type === "text" ? "text" : "vision";
    ...
    const normalizedStatus = normalizedApiKey ? "active" : "inactive";
