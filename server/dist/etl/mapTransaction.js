...
function parseAmount(raw) {
    ...
    return { ok: true, sign, abs: abs.toFixed(2) };
}
function parseDirection(raw) {
    const v = raw.trim();
    if (v.includes("支"))
        return "EXPENSE";
    if (v.includes("收") || v.includes("入"))
        return "INCOME";
    return null;
}
function inferLoanLikeType(params) {
    ...
    if (source === "alipay" && originalCategory.includes("信用借还")) {
        if (combined.includes("还款")
            || combined.includes("归还")
            || statusRaw.includes("还款成功")) {
            return "REPAYMENT";
        }
        if (combined.includes("放款")
...
function getUnifiedStatus(status, source) {
...
function mapCategory(category, source) {
    ...
    // 检查是否是有效的统一分类
    if (UNIFIED_CATEGORIES.includes(category)) {
        return category;
    }
    // 如果都不匹配，使用原始分类（可能是支付宝的分类，直接用）
    return category || "其他";
}
export function mapRowToTransaction(row, source) {
    ...
    const inferredType = direction === "EXPENSE"
        ? "EXPENSE"
        : direction === "INCOME"
            ? "INCOME"
            : amountInfo.sign < 0
                ...
                : amountInfo.sign > 0
                    ...
                    : "EXPENSE"; // 默认当作支出
    // 获取原始分类并映射到统一分类
    const originalCategory = source === "wechat"
