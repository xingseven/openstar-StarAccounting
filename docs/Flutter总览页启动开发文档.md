...
**旧 Next.js 外壳 + 新 Flutter 总览内容**

具体做法是：

1. 保留原有 `web/` 的登录体系、主布局、侧边栏、Header 和其余业务页面
2. 给 Flutter 新增一个无壳层的嵌入路由：`/embed/dashboard`
3. 使用 `flutter build web --base-href /flutter-dashboard/` 生成 Flutter Web 静态包
4. 将构建产物同步到：
   ```text
   web/public/flutter-dashboard/
   ```
5. 把当前 `web/src/app/(dashboard)/page.tsx` 改为加载新的 Flutter 总览页
...
所以现在不是“整站已经 Flutter 化”，而是：

**先把总览页切到新实现，旧站壳层先继续顶住。**
...
- Next.js 当前总览入口切到新页面后，没有引入新的前端构建错误

---

## 13. 后续建议

既然当前总览页已经切过去了，后续建议按这个顺序继续：
...
- 资产页和数据页更适合延续当前“旧站壳层 + 新 Flutter 内容”的策略
- 消费页最复杂，放在第三步更稳

这意味着接下来不应该再回去重做总览页，而是直接在当前切换基础上继续迁下一个页面。
