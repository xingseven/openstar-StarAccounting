...
export function detectLargeExpenses(transactions, config = DEFAULT_CONFIG) {
    const anomalies = [];
...
export function detectHighFrequency(transactions, config = DEFAULT_CONFIG) {
    const anomalies = [];
    const now = new Date();
...
export function detectUnusualPatterns(transactions, config = DEFAULT_CONFIG) {
    const anomalies = [];
    const now = new Date();
    const recentDate = new Date(now.getTime() - config.recentDays * 24 * 60 * 60 * 1000);
    const recentTx = transactions.filter((tx) => {
    ...
    for (const tx of olderTx) {
        if (tx.category)
            olderCategories.add(tx.category);
    }
...
export function detectAnomalies(transactions, config = DEFAULT_CONFIG) {
